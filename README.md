...
```
	...
	when mouse hover on clickable, hello world will change to name of app (single line), and transition in a cool way
		...
		now, every letter, is going to accelerate towards the logo while changing their direction in a certain way, and once they get inside the logo, instantly (not fully instantly, but at the speed it takes to travel through the logo) they come back out as another letter (because there is mismatch of numbers, the speeds will be different so multiple letters get to the same logo at the same time), which is still flying out in the direction it came out in, and decelerates towards its correct position
